...
const AboutPage = () => {
  ...
  const [settings, setSettings] = useState<AboutPageSettings>({
    ...
    about_specializations: [
      ...
      { icon: "ri-film-line", label: "AI Film Editing" },
  ...
  const loadSettings = async () => {
    try {
      const { data } = await supabase
        .from("site_settings")
        .select("about_name, about_title, about_bio_2, about_years_exp, about_photo_url, about_specializations, about_timeline")
        .eq("id", 1)
        .maybeSingle();
      if (data) {
        setSettings({
  ...
  const { about_name, about_title, about_bio_2, about_years_exp, about_photo_url, about_specializations, about_timeline } = settings;

  const specializationsWithDesc = about_specializations.map((s, i) => {
    const descs = [
      "توليد صور احترافية بجودة تجارية",
      "إنتاج مشاهد سينمائية بالذكاء الاصطناعي",
      "بناء برومبتات احترافية محسّنة",
      "إخراج بصري وهوية بصرية متكاملة",
    ];
    return { ...s, desc: descs[i] || "" };
  });
      ...
      <section className="py-20 px-4 md:px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {specializationsWithDesc.map((spec) => (
