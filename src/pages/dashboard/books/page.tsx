...
interface StoreFeature { title: string; desc: string; }
interface StoreFaq { q: string; a: string; }
...
const DashboardBooksPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<"books" | "orders" | "store">("books");
  ...
  // Store content management state
  const [storeContent, setStoreContent] = useState<StoreContent>({
    store_hero_title: "خبرتي في",
    store_hero_title_highlight: "يدك",
    store_hero_subtitle: "",
    store_features: [],
    store_faqs: [],
  });
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const safeName = `book-${Date.now()}-${Math.random().toString(36).slice(2)}.${fileExt}`;
      const { data, error: uploadError } = await supabase.storage.from("public").upload(safeName, file, {
        cacheControl: "3600",
        upsert: false,
      });
      ...
      if (signedError || !signedData?.signedUrl) throw signedError || new Error("فشل إنشاء رابط الصورة");
      setForm({ ...form, cover_url: signedData.signedUrl });
    } catch (err: unknown) {
  ...
  const openEditModal = (book: Book) => {
    ...
    setForm({
      ...
      buy_url: book.buy_url || "", category: book.category || "",
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      buy_url: form.buy_url || null, category: form.category || null,
  ...
  const fetchStoreContent = useCallback(async () => {
    ...
    try {
      const { data, error: fetchError } = await supabase
        .from("site_settings")
        .select("store_hero_title, store_hero_title_highlight, store_hero_subtitle, store_features, store_faqs")
        .eq("id", 1)
        .maybeSingle();
      if (fetchError) throw fetchError;
      if (data) {
        setStoreContent({
          store_hero_title: data.store_hero_title || "خبرتي في",
          store_hero_title_highlight: data.store_hero_title_highlight || "يدك",
          store_hero_subtitle: data.store_hero_subtitle || "",
          store_features: data.store_features || [],
          store_faqs: data.store_faqs || [],
        });
      }
    } catch (err: unknown) {
      setStoreMessage({ type: "error", text: err instanceof Error ? err.message : "فشل تحميل محتوى المتجر" });
    } finally {
  ...
  useEffect(() => {
    if (activeTab === "store") fetchStoreContent();
  }, [activeTab, fetchStoreContent]);
  ...
  const handleSaveStore = async () => {
    ...
    try {
      const { error: saveError } = await supabase.from("site_settings").upsert({
        id: 1,
        store_hero_title: storeContent.store_hero_title,
        store_hero_title_highlight: storeContent.store_hero_title_highlight,
        store_hero_subtitle: storeContent.store_hero_subtitle,
        store_features: storeContent.store_features,
        store_faqs: storeContent.store_faqs,
        updated_at: new Date().toISOString(),
      });
      if (saveError) throw saveError;
      showToast("تم حفظ محتوى المتجر بنجاح", "success");
      setStoreMessage({ type: "success", text: "تم حفظ محتوى المتجر بنجاح" });
    } catch (err: unknown) {
      setStoreMessage({ type: "error", text: err instanceof Error ? err.message : "فشل حفظ محتوى المتجر" });
    } finally {
      setStoreSaving(false);
    }
  };

  const updateStoreField = (field: "store_hero_title" | "store_hero_title_highlight" | "store_hero_subtitle", value: string) => {
    setStoreContent((prev) => ({ ...prev, [field]: value }));
  };

  const updateStoreFeature = (index: number, field: "title" | "desc", value: string) => {
  ...
  const addStoreFeature = () => {
    setStoreContent((prev) => ({ ...prev, store_features: [...prev.store_features, { title: "", desc: "" }] }));
  };

  const removeStoreFeature = (index: number) => {
    setStoreContent((prev) => ({ ...prev, store_features: prev.store_features.filter((_, i) => i !== index) }));
  };

  const updateStoreFaq = (index: number, field: "q" | "a", value: string) => {
    setStoreContent((prev) => {
      const updated = [...prev.store_faqs];
      updated[index] = { ...updated[index], [field]: value };
      return { ...prev, store_faqs: updated };
    });
  };

  const addStoreFaq = () => {
    setStoreContent((prev) => ({ ...prev, store_faqs: [...prev.store_faqs, { q: "", a: "" }] }));
  };

  const removeStoreFaq = (index: number) => {
    setStoreContent((prev) => ({ ...prev, store_faqs: prev.store_faqs.filter((_, i) => i !== index) }));
  };
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex gap-1 bg-white/5 border border-white/10 rounded-xl p-1 w-fit mb-5">
        ...
        <button onClick={() => setActiveTab("store")} className={`px-5 py-2 rounded-lg text-sm font-medium transition-all cursor-pointer whitespace-nowrap ${activeTab === "store" ? "bg-blue-600 text-white" : "text-white/50 hover:text-white"}`}>محتوى المتجر</button>
      </div>
                      ...
                      <tr key={book.id} className="border-t border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            <button onClick={() => navigate(`/dashboard/books/${book.id}`)} className="w-8 h-8 flex items-center justify-center text-blue-400 hover:text-blue-300 hover:bg-blue-600/20 rounded-lg transition-colors cursor-pointer" title="عرض التفاصيل"><i className="ri-eye-line text-sm"></i></button>
                            <button onClick={() => openEditModal(book)} className="w-8 h-8 flex items-center justify-center text-amber-400 hover:text-amber-300 hover:bg-amber-600/20 rounded-lg transition-colors cursor-pointer" title="تعديل"><i className="ri-edit-line text-sm"></i></button>
                            <button onClick={() => setDeleteConfirm(book.id)} className="w-8 h-8 flex items-center justify-center text-red-400 hover:text-red-300 hover:bg-red-500/20 rounded-lg transition-colors cursor-pointer" title="حذف"><i className="ri-delete-bin-line text-sm"></i></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      {activeTab === "store" && (
        <div dir="rtl" className="space-y-6 max-w-3xl">
          {storeMessage && (
            <div className={`border rounded-xl p-4 flex items-center gap-3 ${storeMessage.type === "success" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-red-500/10 border-red-500/20 text-red-400"}`}>
              <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                <i className={storeMessage.type === "success" ? "ri-checkbox-circle-line" : "ri-error-warning-line"}></i>
              </div>
            ...
            <>
              <div className="bg-[#0d1b2e] border border-white/10 rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <h3 className="font-bold text-white">محتوى صفحة المتجر</h3>
                </div>
                <p className="text-white/30 text-xs mb-5">تحكم في عنوان ووصف صفحة المتجر، المزايا، والأسئلة الشائعة</p>

                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-white/70 mb-1.5">عنوان المتجر</label>
                      <input type="text" value={storeContent.store_hero_title} onChange={(e) => updateStoreField("store_hero_title", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" placeholder="خبرتي في" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-white/70 mb-1.5">الكلمة المظللة في العنوان</label>
                      <input type="text" value={storeContent.store_hero_title_highlight} onChange={(e) => updateStoreField("store_hero_title_highlight", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" placeholder="يدك" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-white/70 mb-1.5">وصف المتجر</label>
                    <textarea value={storeContent.store_hero_subtitle} onChange={(e) => updateStoreField("store_hero_subtitle", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500 resize-none" rows={3} maxLength={300} placeholder="كتب إلكترونية، مكتبات برومبتات..." />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0d1b2e] border border-white/10 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-3">
                  <label className="text-sm font-medium text-white/70">المزايا (مثل: تحميل فوري)</label>
                  <button onClick={addStoreFeature} className="text-blue-400 hover:text-blue-300 text-xs font-medium flex items-center gap-1 cursor-pointer whitespace-nowrap">
                ...
                <div className="space-y-3">
                  {storeContent.store_features.map((f, index) => (
                    <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-4">
                      <div className="flex items-start gap-3">
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            <label className="block text-sm font-medium text-white/70 mb-1.5">العنوان</label>
                            <input type="text" value={f.title} onChange={(e) => updateStoreFeature(index, "title", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" placeholder="تحميل فوري" />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-white/70 mb-1.5">الوصف</label>
                            <input type="text" value={f.desc} onChange={(e) => updateStoreFeature(index, "desc", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" placeholder="احصل على ملفاتك مباشرة" />
                          </div>
                        </div>
                        <button onClick={() => removeStoreFeature(index)} className="w-9 h-9 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 flex items-center justify-center transition-colors cursor-pointer flex-shrink-0" title="حذف">
                          <i className="ri-delete-bin-line"></i>
                        </button>
                      </div>
                    </div>
                  ))}
                  {storeContent.store_features.length === 0 && (
                    <p className="text-white/30 text-sm text-center py-3">لا توجد مزايا بعد</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-[#0d1b2e] border border-white/10 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-3">
                  <label className="text-sm font-medium text-white/70">الأسئلة الشائعة</label>
                  <button onClick={addStoreFaq} className="text-blue-400 hover:text-blue-300 text-xs font-medium flex items-center gap-1 cursor-pointer whitespace-nowrap">
                    <div className="w-4 h-4 flex items-center justify-center"><i className="ri-add-line"></i></div>
                    إضافة سؤال
                  </button>
                </div>
                ...
                <div className="space-y-3">
                  {storeContent.store_faqs.map((f, index) => (
                    <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-4">
                      <div className="flex items-start gap-3">
                        <div className="flex-1 space-y-3">
                          <div>
                            <label className="block text-sm font-medium text-white/70 mb-1.5">السؤال</label>
                            <input type="text" value={f.q} onChange={(e) => updateStoreFaq(index, "q", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500" placeholder="ما صيغة الملفات؟" />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-white/70 mb-1.5">الإجابة</label>
                            <textarea value={f.a} onChange={(e) => updateStoreFaq(index, "a", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-500 resize-none" rows={2} placeholder="الإجابة..." />
                          </div>
                        </div>
                        <button onClick={() => removeStoreFaq(index)} className="w-9 h-9 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 flex items-center justify-center transition-colors cursor-pointer flex-shrink-0" title="حذف">
                          <i className="ri-delete-bin-line"></i>
                        </button>
                      </div>
                    </div>
                  ))}
                  {storeContent.store_faqs.length === 0 && (
                    <p className="text-white/30 text-sm text-center py-3">لا توجد أسئلة بعد</p>
                  )}
                </div>
              </div>

              <div className="flex items-center gap-4 pt-2 pb-8">
                <button onClick={handleSaveStore} disabled={storeSaving} className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold px-8 py-3 rounded-xl transition-colors cursor-pointer whitespace-nowrap text-sm">
                  {storeSaving ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 cursor-pointer">
                    ...
                    {bookCategories.map((c) => (<option key={c} value={c} className="bg-[#0d1b2e]">{c}</option>))}
                  </select>
                </div>
                <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {form.cover_url && (
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <span className="text-xs text-emerald-400 truncate">✓ تم الرفع</span>
