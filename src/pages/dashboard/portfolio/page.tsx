...
const DashboardPortfolioPage = () => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const safeName = `portfolio-${Date.now()}-${Math.random().toString(36).slice(2)}.${fileExt}`;
      ...
      setForm({ ...form, image_url: signedData.signedUrl });
    } catch (err: unknown) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowModal(false);
      showToast(editingId ? "تم تعديل العمل بنجاح" : "تم إضافة العمل بنجاح", "success");
      await fetchItems();
    } catch (err: unknown) {
                    ...
                    <tr key={item.id} className="border-t border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <button onClick={() => navigate(`/dashboard/portfolio/${item.id}`)} className="w-8 h-8 flex items-center justify-center text-blue-400 hover:text-blue-300 hover:bg-blue-600/20 rounded-lg transition-colors cursor-pointer" title="عرض التفاصيل"><i className="ri-eye-line text-sm"></i></button>
                          <button onClick={() => openEditModal(item)} className="w-8 h-8 flex items-center justify-center text-amber-400 hover:text-amber-300 hover:bg-amber-600/20 rounded-lg transition-colors cursor-pointer" title="تعديل"><i className="ri-edit-line text-sm"></i></button>
                          <button onClick={() => setDeleteConfirm(item.id)} className="w-8 h-8 flex items-center justify-center text-red-400 hover:text-red-300 hover:bg-red-500/20 rounded-lg transition-colors cursor-pointer" title="حذف"><i className="ri-delete-bin-line text-sm"></i></button>
                        </div>
                      </td>
                    </tr>
                  );
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 cursor-pointer">
                    ...
                    {categories.map((c) => (<option key={c} value={c} className="bg-[#0d1b2e]">{c}</option>))}
                  </select>
                </div>
                <div>
