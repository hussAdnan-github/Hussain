...
const DashboardPromptsPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowModal(false);
      showToast(editingId ? "تم تعديل البرومبت بنجاح" : "تم إضافة البرومبت بنجاح", "success");
      await fetchPrompts();
                  ...
                  <tr key={prompt.id} className="border-t border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => navigate(`/dashboard/prompts/${prompt.id}`)} className="w-8 h-8 flex items-center justify-center text-blue-400 hover:text-blue-300 hover:bg-blue-600/20 rounded-lg transition-colors cursor-pointer" title="عرض التفاصيل"><i className="ri-eye-line text-sm"></i></button>
                        <button onClick={() => openEditModal(prompt)} className="w-8 h-8 flex items-center justify-center text-amber-400 hover:text-amber-300 hover:bg-amber-600/20 rounded-lg transition-colors cursor-pointer" title="تعديل"><i className="ri-edit-line text-sm"></i></button>
                        <button onClick={() => setDeleteConfirm(prompt.id)} className="w-8 h-8 flex items-center justify-center text-red-400 hover:text-red-300 hover:bg-red-500/20 rounded-lg transition-colors cursor-pointer" title="حذف"><i className="ri-delete-bin-line text-sm"></i></button>
                      </div>
                    </td>
                  </tr>
                ))}
