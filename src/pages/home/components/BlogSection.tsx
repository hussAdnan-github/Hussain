...
import { defaultHomeSections, mergeHomeSections, type BlogTexts } from "@/lib/homeSections";
...
const BlogSection = () => {
  ...
  const [texts, setTexts] = useState<BlogTexts>(defaultHomeSections.blog);
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (settingsRes.data?.home_sections) {
          setTexts(mergeHomeSections(settingsRes.data.home_sections).blog);
        }
      } catch (err: any) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-blue-600 text-sm font-semibold">{texts.badge}</span>
            </div>
