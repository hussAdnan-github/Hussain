...
import { defaultHomeSections, mergeHomeSections, type BooksTexts } from "@/lib/homeSections";
...
const BooksSection = () => {
  ...
  const [texts, setTexts] = useState<BooksTexts>(defaultHomeSections.books);
    ...
    const fetchBooks = async () => {
      try {
        ...
        if (settingsRes.data?.home_sections) {
          setTexts(mergeHomeSections(settingsRes.data.home_sections).books);
        }
      } catch (err: any) {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-blue-600 text-xs font-semibold tracking-wider uppercase">{texts.badge}</span>
            </div>
