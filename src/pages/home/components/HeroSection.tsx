...
const HeroSection = () => {
  ...
  const [settings, setSettings] = useState<HeroSettings>({
    hero_title: "من فكرة خام إلى مشهد سينمائي جاهز للنشر باستخدام AI",
    hero_title_highlight: "مشهد سينمائي",
    hero_subtitle: "أصنع صورًا واعلانات سينمائية بالذكاء الاصطناعي للعلامات التجارية وصناع المحتوى. بجودة عالية وتكلفة أقل.",
    hero_badge: "فني AI",
    hero_photo_url: "https://storage.readdy-site.link/project_files/89593955-b165-4908-8cef-673a8d5d832b/53fb9054-62a0-41bc-8464-18a6e2a73acb_hf_20260123_215330_37846dd6-76eb-434c-977b-73104a61e09f-copy.png?v=7c51afc402940992b54af964c096aa25",
    hero_tools: [
      { name: "Nano Banana", icon: "ri-image-2-line" },
      { name: "Seedance", icon: "ri-film-line" },
      { name: "Kling", icon: "ri-video-line" },
      { name: "Premiere", icon: "ri-scissors-line" },
    ],
  });
  ...
  const loadSettings = async () => {
    try {
      const { data } = await supabase.from("site_settings").select("hero_title, hero_title_highlight, hero_subtitle, hero_badge, hero_photo_url, hero_tools").eq("id", 1).maybeSingle();
      if (data) {
        setSettings({
          hero_title: data.hero_title || settings.hero_title,
          hero_title_highlight: data.hero_title_highlight || settings.hero_title_highlight,
          hero_subtitle: data.hero_subtitle || settings.hero_subtitle,
          hero_badge: data.hero_badge || settings.hero_badge,
          hero_photo_url: data.hero_photo_url || settings.hero_photo_url,
          hero_tools: data.hero_tools || settings.hero_tools,
        });
      }
    } catch { /* keep defaults */ }
  };
  ...
  const { hero_title, hero_title_highlight, hero_subtitle, hero_badge, hero_photo_url, hero_tools } = settings;
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          <div className="flex-1 text-center lg:text-right">
            <div className="inline-flex items-center gap-2 bg-blue-600/20 border border-blue-500/30 rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-blue-300 text-sm font-medium">{hero_badge}&nbsp;</span>
            </div>

            <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-white leading-tight mb-4">
              {hero_title.replace(hero_title_highlight, "")}
