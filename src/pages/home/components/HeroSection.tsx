    ...
    (async () => {
      try {
        const [settingsRes, statsRes] = await Promise.all([
          supabase
            .from("site_settings")
            .select("hero_title, hero_title_highlight, hero_subtitle, hero_badge, hero_photo_url, hero_tools")
            .eq("id", 1)
            .maybeSingle(),
          ...
          <div className="flex-1 text-center lg:text-right">
            {loading ? (
              <div className="animate-pulse space-y-5">
                <div className="h-9 bg-white/10 rounded-full w-44 mx-auto lg:mx-0"></div>
                <div className="space-y-3">
                  <div className="h-10 bg-white/10 rounded-lg w-3/4 mx-auto lg:mx-0"></div>
                  <div className="h-10 bg-white/10 rounded-lg w-1/2 mx-auto lg:mx-0"></div>
                </div>
          ...
          <div className="flex-1 hidden lg:flex justify-center lg:justify-end">
            {!loading && hero_photo_url ? (
              ...
              <div className="relative w-full max-w-lg animate-pulse">
                <div className="rounded-3xl bg-white/10" style={{ height: "480px" }}></div>
              </div>
            )}
          </div>
        </div>
